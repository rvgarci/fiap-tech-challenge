...
from app.models.models import CommercialItemModel, ExportItemModel, ImportItemModel, ProcessingItemModel, ProductionItemModel
from app.services.db_writer import store_data
from app.services.scraper import get_commercial_data, get_export_data, get_import_data, get_processing_data, get_production_data
...
def load_all_data(db: Session = Depends(get_db)):
    ...
    for ano in anos:
        for sub in suboptions_proc:
            store_data(get_processing_data("processamento", sub, ano), ProcessingItemModel, sub)
    
    # Comercialização
    for ano in anos:
        store_data(get_commercial_data("comercializacao", None, ano), CommercialItemModel)
